...
class MobileNetV2Attribute(BaseFaceAttribute):
    ...
    def _load_model(self) -> Any:
        ...
        if missing or unexpected:
            print(f"[attribute] state_dict mismatch — "
    ...
    def _preprocess(self, crop_bgr: np.ndarray) -> np.ndarray:
        ...
        rgb = (rgb - np.array(self.IMAGENET_MEAN, dtype=np.float32)) / \
              np.array(self.IMAGENET_STD, dtype=np.float32)
    ...
    def predict(self, image: np.ndarray,
        ...
        age_idx_arr = torch.arange(age_logits.size(1),
    ...
    def visualize(self, image: np.ndarray,
        ...
        out = image.copy()
        for r in results:
            x1, y1, x2, y2 = (int(r.bbox.x1), int(r.bbox.y1),
                              int(r.bbox.x2), int(r.bbox.y2))
